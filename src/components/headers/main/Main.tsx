 ...
 */
const MainHeader = () => {
  const [showApps, toggleApps] = useToggleState(false);
  const [showNotifications, toggleNotifications] = useToggleState(false);
  const [showUserMenu, toggleUserMenu] = useToggleState(false);

  // Memoized handlers to prevent unnecessary re-renders
  const handleToggleApps = useCallback(() => toggleApps(), [toggleApps]);
    ...
    <nav className="bg-white border-b border-gray-200 px-4 py-2.5 dark:bg-gray-800 dark:border-gray-700 fixed left-0 right-0 top-0 z-50">
      <div className="flex flex-wrap justify-between items-center">
        ...
        <section className="flex items-center lg:order-2">
          ...
          <UserMenuDropdown isOpen={showUserMenu} />
        </section>
      </div>
    </nav>
  );
